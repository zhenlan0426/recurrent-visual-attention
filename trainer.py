...
factor = 0.2
T = 6
line_range = torch.arange(1,T+1,device='cuda').flip(0)
def reverse_cummean(a,axis=1):
    # use mean instead of sum, as the V(s) depends on t as well as the episode is finite
    # otherwise V and pi: S -> A should both depends on t
    return a.flip(axis).cumsum(axis).flip(axis)/line_range
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                for t in range(self.num_glimpses):
                    ...
                    h_t, l_t, b_t, log_prob, p = self.model(x, l_t, h_t, last=True)
                    # store
                    locs.append(l_t[0:9])
                ...
                # calculate reward
                b_, t_, c_ = log_probas.shape
                R = self.loss_fn(log_probas.reshape(b_*t_,c_),\
                                 y.reshape(b_,1).broadcast_to(b_,t_).reshape(b_*t_)).reshape(b_,t_)
                
                # compute losses for differentiable modules
                loss_action = R.mean()
                R_cum = reverse_cummean(R.detach())
                loss_baseline = F.mse_loss(baselines, R_cum, reduction='mean')

                # compute reinforce loss
                # summed over timesteps and averaged across batch
                adjusted_reward = R_cum - baselines.detach()
                loss_reinforce = -(log_pi*adjusted_reward).mean()

                # sum up into a hybrid loss
                loss = loss_action + loss_baseline * factor + loss_reinforce * factor

                # compute accuracy
                #correct = log_probas.mean(1).argmax(1) == y
                correct = log_probas[:,-1].argmax(1) == y
                ...
                loss.backward()
                #torch.nn.utils.clip_grad_value_(self.model.parameters(), self.lr*8)
                self.optimizer.step()
